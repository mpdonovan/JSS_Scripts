...
###DO NOT USE STILL IN DEVELOPMENT THIS DOES NOT WORK
...
if [ -n "$bootCampStatus" ]; then
	...
	diskutil eraseVolume JHFS+ kisdtemp /dev/$bootCampStatus
...
case $value in
'2011 Win7 50M_50W')
	echo "2011 Win7 50M_50W"
	if [[ $comModel == "MacBookPro8,1" || $comModel == "MacBookPro8,2" || $comModel == "iMac12,1" ]]; then
		echo "Model is Compatable"
		modelStatus="Compatable"
		installPackage="/Volumes/a-share-name/2011/win7oobe.pkg"
	else
